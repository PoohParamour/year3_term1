var TEMPLATES = (function(){
	var 
		expose = {
			...
			siteadminSuccess: siteadminSuccess,
			siteadminFailure: siteadminFailure,
	...
	function siteadmin(){
		console.info("showing the site admin page");
		var html_str = '';
		html_str += '<section>';
		html_str += 	'<p>Welcome to Administrators page </p>';
		html_str += 	'<p> You need to login using your Administrator Credentials to access the contents </p>';
		html_str += 	'<span data-action="login">Admin login</span>';
		html_str += '</section>';

		$("[data-role='page_header'] > h1").text("admin page");
		$("[data-role='page_content']").html(html_str);
		$("[data-target]").removeAttr("data-selected");
		$("[data-target='siteadmin']").attr("data-selected", "selected");

		var params = {

		};

		AJAXER.post("siteadmin", params, siteadminSuccess, siteadminFailure);

	}

	function siteadminFailure(e, response){
	...
	function siteadminSuccess(response){
		var html_str = '';
		//separating username and group name from the string
		//
		if (response.cognito_username_str.search("-") >0) {
			response.cognito_username_str = response.cognito_username_str.substring(0,response.cognito_username_str.search("-"));
		};	
		if (response.cognito_username_str == 'admin') {
			if(response && response.cognito_username_str){
				html_str += 	'<section>';
				html_str += 		'<p>Site administration</p>';
//				html_str += 		<p>response<p>;
				html_str += 		'<span data-role="loggedin_in_student_name">Hello ' + response.cognito_username_str + '</span>';
//				html_str += 		'<span data-role="loggedin_in_student_name"> printing response ' + response + '</span>';				
				html_str += 		'<span data-action="logout">logout</span>';
				html_str += 		'<section data-role="siteadmin_container">';
				html_str += 			'<p>You are logged in as Admin.</p>';
				//html_str +=			'<p>' + response + '</p>';
	...
	//			html_str += 			'<output data-role="validate_credentials_output"></output>'; //add data returned from DDB API
				html_str += 			'<p> Admin page under construction.</p>';
			...
			html_str += '<h2>You need Admin credentials to see Admin page</h2>';
			return HELPER.showProblemModal(html_str);
		}
	}
