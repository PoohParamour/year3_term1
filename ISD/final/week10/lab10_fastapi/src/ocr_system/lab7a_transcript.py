...
# รองรับทั้งการรันไฟล์โดยตรงและการ import เป็นแพ็กเกจจาก Lab 8
try:
...
# ไฟล์ภาพจาก Lab 8 อาจใหญ่กว่าภาพ PDF @ 150 DPI มาก แม้มีข้อความน้อย
# จำกัดด้านยาวเฉพาะสำเนาที่ส่งเข้า VLM; ไฟล์ผลลัพธ์ต้นฉบับยังคงความละเอียดเดิม
MAX_IMAGE_SIDE = int(os.getenv("LAB7_MAX_IMAGE_SIDE", "1800"))

# ค่าเดิม 16384/8192 สูงเกินจำเป็นสำหรับ transcript สั้นและกิน VRAM โดยไม่ช่วย OCR
NUM_CTX = int(os.getenv("LAB7_NUM_CTX", "4096"))
NUM_PREDICT = int(os.getenv("LAB7_NUM_PREDICT", "2048"))
...
def load_pages(path: str) -> list[bytes]:
    ...
    if p.suffix.lower() in {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff"}:
        ...
        print(f"  ย่อสำเนาสำหรับ VLM: {new_size[0]}x{new_size[1]} px")
        return [buf.getvalue()]
...
# Typhoon-OCR 1.5 เป็น task-specific model และต้องใช้ prompt ของ v1.5 ให้ครบ
# prompt แบบสั้นของ Typhoon OCR v1 ทำให้รุ่น 1.5 echo คำสั่งกลับมา
# หรือคืน LaTeX ที่ไม่ใช่เนื้อหาเอกสาร
...
def ollama_chat(model: str, messages: list[dict], *, fmt: dict | None = None,
    ...
    if think is not None:
        # Qwen3 เปิด reasoning โดยปริยาย แต่งานคัดลอกลง JSON Schema
        # ไม่ต้องใช้ chain-of-thought ซึ่งทำให้ช้เวลาและ output token เกินจำเป็น
        payload["think"] = think
...
def ocr_pages_to_markdown(pages: list[bytes]) -> str:
    """อ่านภาพทุกหน้าด้วย Typhoon-OCR แล้วคืน Markdown รวม"""
    md_parts: list[str] = []
    for i, png in enumerate(pages):
...
def structure_markdown(document_text: str) -> dict:
    """จัด Markdown ที่ OCR แล้วเป็น JSON ด้วย text LLM"""
...
def pipeline_vlm(pages: list[bytes], save_md: Path | None = None) -> dict:
    ...
    """
    document_text = ocr_pages_to_markdown(pages)
    ...
    return structure_markdown(document_text)
...
def main(argv: list[str] | None = None) -> dict[str, dict]:
    """รัน Lab 7A และคืนผลของแต่ละ pipeline เพื่อให้แล็บอื่นเรียกใช้ซ้ำได้"""
